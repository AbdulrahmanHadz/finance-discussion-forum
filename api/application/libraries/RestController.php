...
class RestController extends \CI_Controller
{
	...
	public function __construct($config = 'rest')
	{
		...
		$this->_supported_formats =
			array_intersect_key($this->_supported_formats, array_flip($supported_formats));
		...
		if ($this->request->format && $this->request->body) {
			$this->request->body =
				Format::factory($this->request->body, $this->request->format)->to_array();
		...
		if ($this->config->item('rest_database_group') &&
		...
		if ($this->input->is_ajax_request() === false && $this->config->item('rest_ajax_only')) {
			...
			$this->response([
				...
				$this->config->item('rest_message_field_name') => $this->lang->line(
					'text_rest_ajax_only'),
	...
	public function _remap($object_called, $arguments = [])
	{
		...
		$log_method =
			!(isset($this->methods[$controller_method]['log']) &&
				$this->methods[$controller_method]['log'] === false);

		// Use keys for this method?
		$use_key =
			!(isset($this->methods[$controller_method]['key']) &&
		...
		if ($this->config->item('rest_enable_keys') && $use_key && $this->_allow === false) {
			...
			$this->response([
				$this->config->item('rest_status_field_name') => false,
				$this->config->item('rest_message_field_name') => sprintf(
		...
		if ($this->config->item('rest_enable_keys') && empty($this->rest->key) === false) {
			...
			$level =
				isset($this->methods[$controller_method]['level']) ? $this->methods[$controller_method]['level'] : 0;
		...
		else if ($this->config->item('rest_limits_method') == 'IP_ADDRESS' &&
			...
			if ($data === null && $http_code === null) {
				...
				if (method_exists(Format::class, 'to_' . $this->response->format)) {
					...
					if (isset($this->_get_args['callback']) &&
						...
						preg_match('/^' . $this->_get_args['callback'] . '/', $output)) {
						$this->output->set_content_type(
							$this->_supported_formats['jsonp'],
	...
	protected function _detect_input_format()
	{
		...
		if (empty($content_type) === false) {
			...
			$content_type =
				(strpos($content_type, ';') !== false ? current(
					explode(';', $content_type)) : $content_type);
	...
	protected function _detect_method()
	{
		...
		method_exists($this, '_parse_' . $method) ? $method : 'get';
	}
	...
	protected function _detect_api_key()
	{
		...
		if (($key =
			isset($this->_args[$api_key_variable]) ? $this->_args[$api_key_variable] : $this->input->server(
				$key_name))) {
			if (!($row =
				$this->rest->db->where($this->config->item('rest_key_column'), $key)->get(
					$this->config->item('rest_keys_table'))->row())) {
				return false;
			}
		...
		$is_inserted = $this->rest->db
			->insert(
				...
				[
					...
					'params' => $this->_args ? ($this->config->item('rest_logs_json_params') ===
					true ? json_encode($this->_args) : serialize($this->_args)) : null,
	...
	protected function _check_limit($controller_method)
	{
		...
		switch ($this->config->item('rest_limits_method')) {
			...
			default:
				...
				$limited_uri =
					...
					$limited_uri .
					':' .
					$this->request->method; // It's good to differentiate GET from PUT
				break;
		}
		...
		$time_limit =
			(isset($this->methods[$controller_method]['time']) ? $this->methods[$controller_method]['time'] : 3600); // 3600 = 60 * 60
	...
	protected function _auth_override_check()
	{
		...
		if (!empty($auth_override_class_method)) {
			...
			if (!empty($auth_override_class_method[$this->router->class][$this->router->method])) {
				...
				if ($auth_override_class_method[$this->router->class][$this->router->method] ===
					'none') {
					return true;
				}
		...
		if (!empty($auth_override_class_method_http)) {
			...
			if (!empty($auth_override_class_method_http[$this->router->class]['*'][$this->request->method])) {
				// None auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method_http[$this->router->class]['*'][$this->request->method] ===
					'none') {
					return true;
				}
			...
			if (!empty($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method])) {
				// None auth override found, prepare nothing but send back a TRUE override flag
				if ($auth_override_class_method_http[$this->router->class][$this->router->method][$this->request->method] ===
					'none') {
					return true;
				}
	...
	public function get($key = null, $xss_clean = null)
	{
		...
		return isset($this->_get_args[$key]) ? $this->_xss_clean(
			$this->_get_args[$key],
			$xss_clean) : null;
	}
	...
	public function options($key = null, $xss_clean = null)
	{
		...
		return isset($this->_options_args[$key]) ? $this->_xss_clean(
			$this->_options_args[$key],
			$xss_clean) : null;
	}
	...
	public function head($key = null, $xss_clean = null)
	{
		...
		return isset($this->_head_args[$key]) ? $this->_xss_clean(
			$this->_head_args[$key],
			$xss_clean) : null;
	}
	...
	public function post($key = null, $xss_clean = null)
	{
		if ($key === null) {
			foreach (new RecursiveIteratorIterator(
				new RecursiveArrayIterator($this->_post_args),
		...
		return isset($this->_post_args[$key]) ? $this->_xss_clean(
			$this->_post_args[$key],
			$xss_clean) : null;
	}
	...
	public function put($key = null, $xss_clean = null)
	{
		...
		return isset($this->_put_args[$key]) ? $this->_xss_clean(
			$this->_put_args[$key],
			$xss_clean) : null;
	}
	...
	public function delete($key = null, $xss_clean = null)
	{
		...
		return isset($this->_delete_args[$key]) ? $this->_xss_clean(
			$this->_delete_args[$key],
			$xss_clean) : null;
	}
	...
	public function patch($key = null, $xss_clean = null)
	{
		...
		return isset($this->_patch_args[$key]) ? $this->_xss_clean(
			$this->_patch_args[$key],
			$xss_clean) : null;
	}
	...
	public function query($key = null, $xss_clean = null)
	{
		...
		return isset($this->_query_args[$key]) ? $this->_xss_clean(
			$this->_query_args[$key],
			$xss_clean) : null;
	}
	...
	protected function _perform_ldap_auth($username = '', $password = null)
	{
		...
		log_message(
			...
			(isset($ldap['host']) ? $ldap['host'] : '[ldap not configured]'));
		...
		if ($ldapconn) {
			...
			log_message(
				'debug',
				'LDAP Auth: Binding to ' . $ldap['host'] . ' with dn ' . $ldap['rdn']);
		...
		if (ldap_count_entries($ldapconn, $res_id) !== 1) {
			log_message(
				'error',
				'LDAP Auth: Failure, username ' . $username . 'found more than once');
		...
		if (($link_id = ldap_bind($ldapconn, $user_dn, $password)) === false) {
			log_message(
				'error',
				'LDAP Auth: Failure, username/password did not match: ' . $user_dn);
	...
	protected function _prepare_basic_auth()
	{
		...
		$http_auth =
			$this->input->server('HTTP_AUTHENTICATION') ?: $this->input->server(
				'HTTP_AUTHORIZATION');
	...
	protected function _prepare_digest_auth()
	{
		...
		preg_match_all(
			...
			$matches);
		$digest =
			(empty($matches[1]) || empty($matches[2])) ? [] : array_combine(
				$matches[1],
				$matches[2]);
